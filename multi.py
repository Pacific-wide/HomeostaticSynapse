...
# Estimator flag
flags.DEFINE_string('model', 'multi', 'Model to evaluate.')
...
def main(unused_argv):
    ...
    for i in range(0, n_task_eval):
        p = p_eval[:i+1]
        eval_run_config = tf.estimator.RunConfig(model_dir="multi-model"+str(i), save_checkpoints_steps=6000)
        estimator = tf.estimator.Estimator(model_fn=model.single, config=eval_run_config, params=params)
        estimator.train(input_fn=lambda: input.train_multi_input_fn(x_tr, y_tr, FLAGS.n_epoch, FLAGS.n_batch, p))
        for j in range(i+1):
